...
function raycastFirst /* @echo INDIRECT_STRING */(bvh, root, side, ray) {
...
function _raycastFirst(nodeIndex32, bvh, side, ray) {
	...
	if (isLeaf) {
		...
		const c1Intersection = intersectRay(
			c1,
			float32Array,
			ray,
			_boxIntersection
		...
		const c2Intersection = intersectRay(
			c2,
			float32Array,
			ray,
			_boxIntersection
